...
import { doCustomTimes, getParams } from '@/utils'
...
export const fetchPages = req => {
  const { name, city, gender, page = 1, pageSize = 10, sort } = JSON.parse(req.body)
...
export const deleteApp = req => {
  let id = req.body
  List = List.filter(element => { return element.id !== id })
  return {
    code: 0,
    data: {
      total: List.length,
